...
function result(id: string, text: string, isError = false): Message {
  return message("user", "", {
    toolResults: [{ tool_use_id: id, text, ...(isError ? { isError: true } : {}) }],
  });
}
    ...
    async ask(state, questions) {
      ...
      return {
        answers: Object.fromEntries(
          Object.keys(questions).map((key) => [key, { noul: answer(key) }]),
        ),
      };
    },
  };
...
const fit = { maxStateTokens: 25_000, preserveRecentMessages: 0, goal: "fix the test" };
...
describe("options", () => {
  it("fills in defaults and ignores non-finite values", () => {
    ...
    ).toMatchObject({ keepThreshold: 0.5, preserveRecentMessages: 2, truncateHeadChars: 0 });
  });
});

describe("token estimate", () => {
  it("charges words, digits and symbols separately and never badly undercounts JSON", () => {
...
describe("state fitting", () => {
  it("sends the whole history with tool results replaced by a note", () => {
    ...
    const failed = state.history[4]?.tool_calls?.[0] as HistoryToolCall | undefined;
    expect(failed?.result).toMatch(/^error, /);
  });
  ...
  it("truncates tool inputs before touching message text", () => {
    const messages = [
      message("user", "start"),
      call("w", "Write", { file_path: "x.ts", content: "x".repeat(5000) }),
      result("w", "ok"),
    ...
    const shrunk = state.history[1]?.tool_calls?.[0] as HistoryToolCall | undefined;
    expect(shrunk?.input.length).toBeLessThanOrEqual(200);
  });
  ...
  it("shrinks old tool calls to one line each when nothing else is left to cut", () => {
    ...
    for (let i = 0; i < 40; i += 1) {
      messages.push(
        call(`c${i}`, "Read", { file_path: `/repo/src/module-${i}.ts` }),
        result(`c${i}`, "x"),
      );
    }
  ...
  it("abridges long texts oldest-first and collapses old messages last", () => {
    ...
    const abridged = fitState(messages, [], {
      ...fit,
      maxStateTokens: 1800,
      preserveRecentMessages: 1,
    });
    ...
    const collapsed = fitState(messages, [], {
      ...fit,
      maxStateTokens: 420,
      preserveRecentMessages: 1,
    });
...
describe("decisions", () => {
  ...
  it("keeps, drops the result, or drops the call based on the keep probabilities", () => {
    ...
    expect(decideCall(unpinned, { keepCall: 0.9, keepResult: 0.2 }, options).action).toBe(
      "drop_result",
    );
    expect(decideCall(unpinned, { keepCall: 0.1, keepResult: 0.2 }, options).action).toBe(
      "drop_call",
    );
    expect(
      decideCall({ ...unpinned, pinned: true }, { keepCall: 0, keepResult: 0 }, options),
    ).toMatchObject({ action: "keep", reason: "pinned" });
  });
  ...
  it("removes dropped calls and truncates dropped results", () => {
    ...
    // The dropped call takes its result with it, so tool-2 survives as a truncated pair.
    expect(kept[2]?.toolUses[0]?.tool_use_id).toBe("tool-2");
    expect(kept[3]?.toolResults?.[0]?.tool_use_id).toBe("tool-2");
    expect(kept[3]?.toolResults?.[0]?.text).toMatch(/^x{300}\n\[jevonian truncated 1700 chars/);
    ...
    short[2]!.toolResults![0]!.text = "y".repeat(100);
    short[5]!.toolResults![0]!.text = "y".repeat(100);
    const shortKept = applyDecisions(short, decisions, calls, 300);
    // A short result is below the truncation budget, so its text is untouched.
    expect(shortKept[3]?.toolResults?.[0]?.text).toBe("y".repeat(100));
  });
  ...
  it("honours truncateHeadChars, including a zero head", () => {
    ...
    expect(kept[2]?.toolResults?.[0]?.text).toBe(
      `${original.slice(0, 50)}\n[jevonian truncated ${total - 50} chars of this tool result; re-run the tool if needed]`,
    );

    const noHead = applyDecisions(messages, decisions, calls, 0);
    expect(noHead[2]?.toolResults?.[0]?.text).toBe(
      `[jevonian truncated ${total} chars of this tool result; re-run the tool if needed]`,
    );
  });
});
...
describe("compact", () => {
  it("resends the full state with every batch and merges the answers", async () => {
    ...
    expect(output.decisions.map((d) => d.action)).toEqual([
      "drop_result",
      "drop_result",
      "drop_result",
    ]);
...
describe("normalizeTranscript", () => {
  ...
  it("pairs calls and results identically across all three wire formats", () => {
    for (const body of [openai, anthropic, responses]) {
      ...
      expect(calls[0]).toMatchObject({
        ...
        resultChars: "file contents".length,
      });
      expect(messages[0]?.text).toBe("fix the bug");
    }
  });

  it("marks a failing result as an error", () => {
    const messages = normalizeTranscript({
      messages: [
        { role: "assistant", content: "", tool_calls: [{ id: "c1", function: { name: "Bash" } }] },
        { role: "tool", tool_call_id: "c1", content: "Error: tests failed" },
      ],
    });
    expect(collectToolCalls(messages, 0)[0]?.isError).toBe(true);
  });
  ...
  it("survives unparseable tool arguments", () => {
    ...
    expect(collectToolCalls(messages, 0)[0]?.input).toEqual({ raw: "{" });
  });
});
