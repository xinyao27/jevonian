...
- `/v1/responses` is proxied for clients that speak the Responses API (Codex CLI). A chat-completions request that routes to a Responses provider is translated on the fly (streaming chunks included), so any OpenAI-compatible agent can use the ChatGPT subscription.
